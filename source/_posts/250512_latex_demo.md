---
title: 基于 LaTeX 生成一份完整的携带者筛查检测报告（共六页）
...
`LaTeX`是一种高质量的排版系统，广泛用于创建科学文档、学术论文和书籍等。本完整示例采用`LaTeX`完整生成了一份 PDF 检测报告，包括 PDF 合并、表格、页眉页脚、固定文本、固定图片、文本框、多行文本、插入图片等等众多细节调整。
